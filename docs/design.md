...
- **M2b — first strike + double strike.** Touches `Turn`. *Double strike is the falsifier*: the obvious implementation is "first strikers in step one, everyone else in step two", which double strike breaks by being in **both**. Also pays off `#19 — see below.
...
**M4g named its own fast-follow, and it landed as M4h.** Modality's
...
activated and triggered abilities could adopt the same payload without a module
cycle. M4h cashed that: `ActivatedAbility`/`TriggeredAbility` both carry a
`modal :: Modal card`, the mode choice is wired into the activation path (CR
602.2b) and trigger placement (CR 700.2b/603.3c/603.3d — including the
trigger-only "if no mode is chosen, the ability is removed from the stack" rule,
which has no spell analog), gated by Aether Channeler, at zero new opcodes. See
`progress.md`'s M4h entry for the gate, fixtures, and every other named deferred
expiry.
...
not yet built, which is exactly the `Synthetic Restart` and
`Synthetic Subgame` pattern already in use. This is the §1 invariant's
