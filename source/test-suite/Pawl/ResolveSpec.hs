    ...
    [ HU.testCase "CR 608 a resolved spell's damage is Noncombat" $ do
      ...
      HU.testCase "CR 701.23 Search fetches a basic land to the battlefield tapped" $ do
        ...
        let base = Setup.emptyGame S.bothPlayers
            ...
            abilObj =
              Object.MkObject S.alice (Source.OfAbility (ObjectId.MkObjectId 0) ability) Zone.Stack TapState.Untapped 0 Sickness.Settled (Binding.fromChoices Map.empty Map.empty Nothing (Set.singleton (ModeIndex.MkModeIndex 0))) Map.empty Nothing ts
      ...
      HU.testCase "CR 701.23b Search may fail to find" $ do
        ...
        let base = Setup.emptyGame S.bothPlayers
            ...
            (abilId, g2) = Game.freshObjectId g1
            (ts, g3) = Game.freshTimestamp g2
            abilObj = Object.MkObject S.alice (Source.OfAbility (ObjectId.MkObjectId 0) ability) Zone.Stack TapState.Untapped 0 Sickness.Settled (Binding.fromChoices Map.empty Map.empty Nothing (Set.singleton (ModeIndex.MkModeIndex 0))) Map.empty Nothing ts
            g4 = g3 {GameState.objects = Map.insert abilId abilObj (GameState.objects g3), GameState.stack = [abilId]}
...
handAppend printing pid gs =
  let (oid, gs1) = Game.freshObjectId gs
      obj = Object.MkObject pid (Source.OfCard printing) Zone.Hand TapState.Untapped 0 Sickness.Settled Map.empty Map.empty Nothing (Timestamp.MkTimestamp 0)
   in ( oid,
        gs1
