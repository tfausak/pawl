      ...
      let base = Setup.emptyGame S.bothPlayers
          ...
          shield =
            ActiveReplacement.MkActiveReplacement
              { ActiveReplacement.effect = ReplacementEffect.DamageR (DamagePattern.MkDamagePattern (Just DamageKind.Combat) (Filter.Type.And []) Nothing) DamageRewrite.PreventAll,
...
sbaSpec s =
  Spec.describe s "Sba" $ do
    ...
    Spec.it s "life <= 0 loses" $
      let gs = sbaBase {GameState.players = Map.insert S.alice (Player.MkPlayer {Player.life = 0, Player.status = Status.Playing, Player.counters = Map.empty, Player.ringTemptations = 0, Player.speed = Nothing, Player.commander = Nothing, Player.commanderCasts = 0, Player.commanderDamage = Map.empty}) (GameState.players sbaBase)}
