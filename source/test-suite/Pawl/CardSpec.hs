...
lintSpec s registry = Spec.describe s "Lint" $ do
  ...
  Spec.it s "the lint itself catches a reserved event slot the condition never binds" $ do
    ...
    let -- Endless Cockroaches' own payload: "return it to its owner's hand".
        returnIt = Effect.MoveToZone (ObjectRef.InSlot Binding.became) Zone.Hand EntryRiders.defaultValue Nothing Nothing LibraryPosition.defaultValue
  ...
  Spec.it s "the lint itself catches a mode reading a slot only another mode declares" $ do
    let creature = SlotName.MkSlotName (Text.pack "creature")
        ...
        exileIt = Effect.MoveToZone (ObjectRef.InSlot creature) Zone.Exile EntryRiders.defaultValue (Just exiled) Nothing LibraryPosition.defaultValue
    ...
    let returnBecame = Effect.MoveToZone (ObjectRef.InSlot Binding.became) Zone.Hand EntryRiders.defaultValue Nothing Nothing LibraryPosition.defaultValue
  ...
  -- #998: no card adds or removes the WORLD supertype. Pawl.Engine.Sba.worldVictims
  -- implements CR 704.5k by reading Object.timestamp -- when the permanent entered
  -- the battlefield -- as a proxy for when it became world, and that proxy is exact
  -- only while nothing moves the supertype on the battlefield. Layer 4 can move it
  -- now (Modification.AddSupertype, RemoveSupertype), so what keeps the proxy honest
  -- is the POOL, and this is what holds the pool to it: the first card authoring
  -- either arm at Supertype.World fails here rather than silently burying the wrong
  -- permanent.
  --
  -- The two world permanents in the pool (Concordant Crossroads, Living Plane) print
  -- the supertype on their type lines, which this does not reach and must not: a
  -- printed supertype arrives with the object and its entry stamp IS its world stamp.
  --
  -- A wildcard rather than an exhaustive case, deliberately: the question is about
  -- one supertype, not about which arms exist, so a Modification added later
  -- correctly answers False without touching this. Casing on Modification at all is
  -- what Pawl.Types.Modification's header licenses a corpus lint to do.
  Spec.it s "no card adds or removes the world supertype (#998)" $ do
    ps <- S.allPrintings s
    let movesWorld m = case m of
          Modification.AddSupertype Supertype.World -> True
          Modification.RemoveSupertype Supertype.World -> True
          _ -> False
        stored effect = case effect of
          Effect.ModifyTarget _ m _ -> [m]
          _ -> []
        -- Both places a Modification can sit on a card: a static ability's list
        -- (CR 604.2) and a resolution's stored effect (CR 611.2).
        faceModifications c =
          concatMap (NonEmpty.toList . StaticAbility.modifications) (Face.staticAbilities c)
            <> concatMap stored (cardResolutionEffects c)
        offenders = filter (anyFace (any movesWorld . faceModifications) . Printing.card) ps
    Spec.assertEqWith s "CR 704.5k's clock is Object.timestamp, which such a card would falsify" (fmap (S.nameOf . Printing.card) offenders) []
    -- Guards against a vacuous sweep: with no supertype modification in the pool at
    -- all this would pass whatever the predicate did. Arcum's Weathervane and
    -- Leyline of Singularity are the cards that print them -- at Snow and Legendary,
    -- which is why they are not offenders.
    let movesAnySupertype m = case m of
          Modification.AddSupertype _ -> True
          Modification.RemoveSupertype _ -> True
          _ -> False
        movers = filter (anyFace (any movesAnySupertype . faceModifications) . Printing.card) ps
    Spec.assertBool s (not (null movers)) "the pool does print supertype modifications, so the sweep is not vacuous"
