...
spec s = Spec.describe s "Pawl.Codec.TriggerCondition" $ do
  ...
  -- round-trip. Not the same wire shape as PlayerAttacks above, whose tag
  -- differs and whose payload is a bare relation, so neither decodes as the
  -- other.
  Spec.it s "PlayerAttacksPlayer round-trips both subjects" $ do
    Common.assertCodec
      s
      TriggerCondition.codec
  ...
  Spec.it s "SpellCast round-trips with its Filter" $
    Common.assertCodec
      s
      TriggerCondition.codec
      (TriggerCondition.SpellCast (SpellCast.MkSpellCast (Filter.And [Filter.ControlledBy PlayerRelation.You, Filter.Or [Filter.HasCardType CardType.Instant, Filter.HasCardType CardType.Sorcery]]) TurnScope.EachTurn Nothing Nothing Nothing))
  ...
  Spec.it s "SpellCast round-trips an opponent's-turn scope" $
    Common.assertCodec
      s
      TriggerCondition.codec
      (TriggerCondition.SpellCast (SpellCast.MkSpellCast (Filter.ControlledBy PlayerRelation.You) TurnScope.OpponentsTurn Nothing Nothing Nothing))
