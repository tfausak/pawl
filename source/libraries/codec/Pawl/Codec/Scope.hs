...
scopeToJson s = case s of
  Scope.InZone z r -> Json.tagged (Text.pack "InZone") (Just (Array (MkArray [Zone.toJson z, PlayerRef.toJson r])))
...
jsonToScope value = do
  ...
  case (Text.unpack t, mv) of
    ("InZone", Just (Array (MkArray [z, r]))) -> Scope.InZone <$> Zone.fromJson z <*> PlayerRef.fromJson r
