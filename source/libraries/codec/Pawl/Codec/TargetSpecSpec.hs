...
spec s = Spec.describe s "Pawl.Codec.TargetSpec" $ do
  ...
  Spec.it s "codecMap keys by slot name, in ascending order" $
    Common.assertCodec
      ...
      TargetSpec.codecMap
      ( Map.fromList
          [ (SlotName.MkSlotName (Text.pack "z-slot"), TargetSpec.required Pool.Players Nothing),
            (SlotName.MkSlotName (Text.pack "a-slot"), TargetSpec.required Pool.Creatures Nothing)
          ]
      )
      """ {"a-slot":{"pool":{"type":"Creatures"}},"z-slot":{"pool":{"type":"Players"}}} """
  Spec.it s "has a schema" $ Common.assertHasSchema s TargetSpec.codec
  Spec.it s "the map has a schema" $ Common.assertHasSchema s TargetSpec.codecMap
