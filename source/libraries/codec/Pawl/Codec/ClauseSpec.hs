...
-- One constructor, so four cases: a populated clause, CR 603.5's `optionality`
...
spec s = Spec.describe s "Pawl.Codec.Clause" $ do
  Spec.it s "MkClause with one effect" $
    Common.assertJsonCodec
      ...
      toJson
      fromJson
      (Clause.MkClause Optionality.Mandatory Nothing (Seq.singleton (Effect.Attach (SlotName.MkSlotName (Text.pack "target")))))
  ...
  Spec.it s "an Optional clause's optionality key is present" $
    Common.assertJsonCodec
      ...
      toJson
      fromJson
      (Clause.MkClause Optionality.Optional Nothing Seq.empty)
  ...
  Spec.it s "an empty clause omits every default field" $
    Common.assertJsonCodec
      ...
      toJson
      fromJson
      (Clause.MkClause Optionality.Mandatory Nothing Seq.empty)
      """ {} """
