...
spec s = Spec.describe s "Pawl.Codec.Condition" $ do
  ...
  Spec.it s "rejects the untagged comparison shape" $
    Spec.assertBool
      s
      (Either.isLeft (Common.parse (Text.pack """ {"measured":{"type":"Power"},"comparison":{"type":"AtLeast"},"threshold":{"type":"Literal","value":1}} """) >>= Condition.fromJson))
      "expected a decode failure"
  Spec.it s "rejects the untagged disjunction shape" $
    Spec.assertBool
      s
      (Either.isLeft (Common.parse (Text.pack """ {"any":[]} """) >>= Condition.fromJson))
      "expected a decode failure"
