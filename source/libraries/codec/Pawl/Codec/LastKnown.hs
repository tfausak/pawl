...
counter :: Codec.Codec (CounterKind.Type.CounterKind Keyword.Type.Keyword, Natural.Natural)
counter = Fields.object $ do
  kind <- Fields.required "kind" (CounterKind.codec Keyword.codec) fst
  count <- Fields.required "count" Common.natural snd
  pure (kind, count)
...
codec = Fields.object $ do
  ...
  counters <- Fields.required "counters" (Common.keyedList counter) LastKnown.counters
