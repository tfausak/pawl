...
-- | A JSON ARRAY of units and not 'Common.multiset', even though CR 106.4 calls
-- a pool a multiset: the type's own haddock says the units are not fungible, so
-- two units differing in provenance are two entries rather than a count of two,
-- and 'Common.set' is wrong for the same reason -- a pool really can hold two
