...
spec s = Spec.describe s "Pawl.Codec.GameEvent" $ do
  ...
  Spec.it s "DamageDealt" $
    Common.assertCodec
      s
      GameEvent.codec
      (GameEvent.DamageDealt (DamageEvent.MkDamageEvent (ObjectId.MkObjectId 1) (Recipient.ToPlayer (PlayerId.MkPlayerId 1)) 2 True False False 3 (Just (PlayerId.MkPlayerId 2)) Nothing mempty False DamageKind.Combat))
  ...
  -- alongside the damage's source and what it prevented for each recipient.
  ...
  Spec.it s "SpellCast" $
    Common.assertCodec
      s
      GameEvent.codec
      (GameEvent.SpellCast (SpellWasCast.MkSpellWasCast (PlayerId.MkPlayerId 0) (ObjectId.MkObjectId 7) ProjectedCharacteristicsSpec.testCharacteristics (Just Zone.Hand) Nothing))
