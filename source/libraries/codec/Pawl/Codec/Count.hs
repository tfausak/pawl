...
countToJson codec (Count.Type.MkCount s f a) =
  Json.tagged (Text.pack "Count") (Just (Array (MkArray [Scope.toJson s, Filter.toJson f, Aggregation.toJson codec a])))

jsonToCount :: (Value -> Either Text q) -> Value -> Either Text (Count.Type.Count q)
jsonToCount decode value = do
  (t, mv) <- Json.tag value
  case (Text.unpack t, mv) of
    ("Count", Just (Array (MkArray [s, f, a]))) -> Count.Type.MkCount <$> Scope.fromJson s <*> Filter.fromJson f <*> Aggregation.fromJson decode a
