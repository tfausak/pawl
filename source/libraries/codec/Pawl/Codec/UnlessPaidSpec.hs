...
spec s = Spec.describe s "Pawl.Codec.UnlessPaid" $ do
  ...
  Spec.it s "an omitted payer field is a decode error" $
    Spec.assertBool
      s
      (Either.isLeft (Codec.decode UnlessPaid.codec (Value.object [Value.pair "cost" (Value.object [Value.pair "mana" (Value.array [])])])))
      "expected a decode failure"
  Spec.it s "an omitted cost field is a decode error" $
    Spec.assertBool
      s
      (Either.isLeft (Codec.decode UnlessPaid.codec (Value.object [Value.pair "payer" (Value.text (Text.pack "spell"))])))
      "expected a decode failure"
  Spec.it s "has a schema" $ Common.assertHasSchema s UnlessPaid.codec
