...
fromJson value = do
  ...
  case (t, mv) of
    ...
    ("Scoped", _) -> Common.withValue mv (fmap GraveyardScope.Scoped . Codec.decode PlayerScope.codec)
    ("InSlot", _) -> Common.withValue mv (fmap GraveyardScope.InSlot . Codec.decode SlotName.codec)
