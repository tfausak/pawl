...
spec s = Spec.describe s "Pawl.Codec.Pool" $ do
  ...
  Spec.it s "Abilities" $
    Common.assertJsonCodec s Pool.toJson Pool.fromJson Pool.Abilities "{\"type\":\"Abilities\"}"
  ...
  Spec.it s "CardsInExile" $
    Common.assertJsonCodec s Pool.toJson Pool.fromJson Pool.CardsInExile "{\"type\":\"CardsInExile\"}"
