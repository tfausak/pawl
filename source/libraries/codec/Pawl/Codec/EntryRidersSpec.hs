...
spec s = Spec.describe s "Pawl.Codec.EntryRiders" $ do
  ...
  Spec.it s "MkEntryRiders, a repeated counter kind is a decode error" $
    Spec.assertBool
      s
      ( case Codec.decode (EntryRiders.codec Common.text) =<< Common.parse (Text.pack "{\"counters\":[{\"kind\":{\"type\":\"PlusOnePlusOne\"},\"count\":{\"type\":\"Literal\",\"value\":1}},{\"kind\":{\"type\":\"PlusOnePlusOne\"},\"count\":{\"type\":\"Literal\",\"value\":2}}]}") of
          Left _ -> True
          Right _ -> False
      )
  ...
  Spec.describe s "defaultValue" $ do
    ...
    Spec.it s "a missing attacking key decodes as Nothing" $
      Common.assertFromJson s (Codec.decode (EntryRiders.codec Common.text)) "{\"tapped\":{\"type\":\"Untapped\"}}" EntryRiders.defaultValue
    ...
    Spec.it s "a missing transformed key decodes as False" $
      Common.assertFromJson s (Codec.decode (EntryRiders.codec Common.text)) "{\"tapped\":{\"type\":\"Untapped\"},\"attacking\":null}" EntryRiders.defaultValue
    ...
    Spec.it s "an explicit null tapped is now a decode error" $
      Spec.assertBool
        s
        ( case Codec.decode (EntryRiders.codec Common.text) (Value.object [Value.pair "tapped" Value.null, Value.pair "attacking" (Value.boolean False)]) of
            Left _ -> True
            Right _ -> False
        )
