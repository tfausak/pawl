...
populatedFace =
  baseFace
    { Face.keywords = Set.singleton Keyword.Deathtouch,
      ...
      Face.replacementEffects = [PrintedReplacement.MkPrintedReplacement Nothing (ReplacementEffect.EntryR (EntryR.MkEntryR Filter.IsSource (EntryRewrite.AsCopy (AsCopy.MkAsCopy (Filter.HasCardType CardType.Creature) [] False)))) Nothing],
      ...
      Face.rooms = Seq.empty,
      Face.playerAbilities = [PlayerStaticAbility.MkPlayerStaticAbility PlayerScope.You Nothing PlayerEffect.CantCastSpells],
...
populatedFaceJson =
  "{\"name\":\"Test Card\",\"manaCost\":[{\"type\":\"Generic\",\"value\":1}],"
    ...
    <> "\"modal\":{\"modes\":[{}]},\"name\":\"activated\"}],"
    <> "\"replacementEffects\":[{\"effect\":{\"type\":\"EntryR\",\"value\":{\"matching\":{\"type\":\"IsSource\"},\"rewrite\":{\"type\":\"AsCopy\",\"value\":{\"eligible\":{\"type\":\"HasCardType\",\"value\":{\"type\":\"Creature\"}}}}}}}],"
  ...
  Spec.describe s "each defaulted field round-trips when present" $ do
    ...
    Spec.it s "playerAbilities" $
      Common.assertJsonCodec
        ...
        encodeFace
        decodeFace
        baseFace {Face.playerAbilities = [PlayerStaticAbility.MkPlayerStaticAbility PlayerScope.You Nothing PlayerEffect.CantCastSpells]}
