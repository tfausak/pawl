...
-- 115.6's "up to one" and every larger count with one range. On the spec and not
...
-- The ordinary "target creature" spec: exactly one recipient. Named because
-- almost every spec in the engine and the corpus is one, so writing the count out
-- at each would bury the handful that are not.
required :: Pool.Pool -> Maybe (Filter.Filter Keyword.Keyword) -> TargetSpec
required p f = MkTargetSpec p f TargetCount.one

-- CR 115.6 / 601.2c's "up to N targets": a slot the caster may fill any number of
-- times up to N, the empty answer included.
upTo :: Natural.Natural -> Pool.Pool -> Maybe (Filter.Filter Keyword.Keyword) -> TargetSpec
upTo n p f = MkTargetSpec p f (TargetCount.upTo n)
