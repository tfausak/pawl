...
-- | CR 701.23: where a search puts what it finds. The searching rule itself
-- says only "look at [a] hidden zone ... to find a card"; every card that
-- searches then says what happens to the card, and those instructions differ.
--
...
data SearchDestination
  = -- | Evolving Wilds: "Search your library for a basic land card, put it onto
    ...
    BattlefieldTapped
  | -- | Braidwood Sextant: "Search your library for a basic land card, reveal
    -- that card, put it into your hand, then shuffle." CR 702.29e's
    -- typecycling says the same sentence in the rulebook's words ("reveal it,
