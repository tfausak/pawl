...
data Binding = MkBinding
  { -- | CR 601.2c: the targets chosen for this slot; each re-validated at CR
    -- 608.2b. A SET and not one Recipient, because one instance of the word
    -- "target" can take several ("up to two target creatures") -- and a set
    -- rather than a sequence because CR 601.2c forbids the same target twice for
    -- one instance, and the game gives the chosen targets no order.
    --
    -- Nothing and never an empty set: a slot the caster declined (CR 115.6) has
    -- no binding at all, which is what keeps CR 608.2b measuring the slots
    -- FILLED.
    targets :: Maybe (Set.Set Recipient.Recipient),
