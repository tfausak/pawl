...
data ProjectedCharacteristics = MkProjectedCharacteristics
  { -- | CR 201.1: the object's names after the layer fold. Copiable (CR 707.2),
    ...
    -- is the chosen name". A Room permanent has one name per unlocked door (CR
    -- 709.5), a split card off the stack has both halves' (CR 709.4), a spell on
    ...
    replacementEffects :: [PrintedReplacement.PrintedReplacement Card.Card (GrantedAbility.GrantedAbility Card.Card) (Effect.Effect Card.Card (GrantedAbility.GrantedAbility Card.Card))],
