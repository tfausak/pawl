...
data Response
  ...
  | -- | The player randomness named for a
    ...
    -- opponent at random", Strax, Sontaran Nurse's "choose a player at random").
    --
    -- Its own constructor rather than ChosePlayer reused, though both name one
    -- player: this type's rule at the top, and here the difference is the whole
    -- point -- a transcript of a player DECIDING must not satisfy a prompt that
    -- asked randomness, which is CR 701.9b's distinction.
    SelectedPlayerAtRandom PlayerId.PlayerId
  ...
  | -- | CR 608.2d: the ONE permanent a player chose out of the ones an effect
    ...
    -- ChoseTaps' reason: replaying a transcript against the wrong one would
    -- attach a permanent where it should have named one to act on.
    ChosePermanent ObjectId.ObjectId
