...
data AttackTarget
  ...
  | -- | CR 306.6. Named by id and nothing else: CR 506.4c can stop it being
    -- attacked while the creature stays in combat, so the readers of this arm ask
    -- whether it is still an attackable planeswalker as they read it
    -- (Pawl.Engine.Combat.stillAttacked) and whether it has already stopped being
    -- one (Pawl.Types.Combat's attackingNothing).
