...
data RollModifier
  = -- | CR 706.2b's FIRST step: throw the same die again and take the new number
    ...
    --
    -- Not implemented: CR 706.2b's SECOND bucket, a modifier that increases or
    -- decreases the result by an amount (#3974); with one bucket occupied there
    -- is nothing for rule 706.2b's ordering to order.
    Reroll
