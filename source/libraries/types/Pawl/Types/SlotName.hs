...
-- read by every ObjectRef-taking opcode, but the remaining bare-SlotName opcodes
-- still project the slot's single target, so one of those naming a group
-- resolves and does nothing (#760).
