...
data Supertype
  = -- | CR 205.4c: any land with this supertype is a basic land, and any land
    ...
    Basic
  | -- | CR 205.4d: subject to CR 704.5j's legend rule, which Pawl.Engine.Sba
    -- implements. CR 205.4e's OTHER rule -- a legendary instant or sorcery
    -- can't be cast unless you control a legendary creature or planeswalker --
    -- is Pawl.Engine.Cast.legendaryRestrictionOk.
    Legendary
  | -- | CR 205.4g: "any permanent with the supertype 'snow' is a snow permanent."
    ...
    Snow
  | -- | CR 205.4f: subject to CR 704.5k's world rule, which Pawl.Engine.Sba
    -- implements (Sba.worldVictims). Unlike the legend rule that one asks
    -- nobody: it keeps the permanent that has had the supertype for the
    -- shortest time, which is a fact rather than a choice.
    World
