...
-- is the printed sentence. Two of them are printed and they ask different
-- questions: "conjure a card named Ornithopter" names a card outright, and
-- "conjure a duplicate of target spell" names an OBJECT ALREADY IN THE GAME and
-- takes its card from there.
--
-- A sum rather than two optional fields on 'Pawl.Types.Conjure.Conjure': no
-- printing says both and none says neither, and 'Pawl.Types.Conjure.selection'
-- is a question only the written road asks.
...
-- card is read off the board. The channel every walk over an opcode\'s NESTED
