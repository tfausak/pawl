...
effectFilters effect = case effect of
  ...
  Effect.PreventAllDamage (PreventAllDamage.MkPreventAllDamage duration _ ref _ chosenSource whatSource rider) ->
    unframed (durationFilters duration <> Maybe.maybeToList chosenSource <> [whatSource]) <> sourceHosted (objectRefFilters ref) <> concatMap effectFilters rider
...
-- cross-check below would read a collapse as a slot the traversal cannot see.
ownTargetSlots :: Face.Face Card.Type.Card -> [TargetSlot.TargetSlot]
ownTargetSlots card =
  Face.enchant card
    <> concatMap
      (concatMap (Map.elems . Mode.targetSlots) . Modal.modes)
      ( Face.spell card
          : ( fmap ActivatedAbility.modal (Face.activatedAbilities card)
...
-- ownTargetSlots over the card's own face AND every face it mints, declaredTargetSlots'
-- recursion and for its reason: a token's ability announces its targets through the
-- same rules the minting card's does.
cardTargetSlots :: Face.Face Card.Type.Card -> [TargetSlot.TargetSlot]
cardTargetSlots card = concatMap ownTargetSlots (card : mintedFaces card)

-- How many target slots on this card name a CR 202.3 computed bound that reads
-- NOTHING, and how many name one that reads a SLOT. The second number is the
-- offence: see the "no card's computed bound reads a slot" case for why a bound
-- naming a slot can never be answered.
--
...
amountSlotCounts card =
  let amounts = Maybe.mapMaybe TargetSlot.amount (cardTargetSlots card)
      (reads_, inert) = List.partition (not . Set.null . Quantity.slots) amounts
   in (length inert, length reads_)

-- TWO offences under one name, canHostSubjectOffends' pair and for its reason: the
-- bound that reads a slot, and a disagreement between ownTargetSlots' hand-kept
-- record fold and the encoding -- which would mean a carrier this traversal cannot
-- see, and a bound sitting in it reported as zero rather than as an offence.
amountOffends :: Face.Face Card.Type.Card -> Bool
amountOffends card =
  let (inert, reads_) = amountSlotCounts card
   in reads_ /= 0 || inert + reads_ /= amountedSlots (Codec.encode (Face.Codec.codec Card.codec) card)

-- How many target slots an encoding holds that NAME an amount. amountedSlotAtoms'
-- walk with the atoms left out, and "pool" identifies a target slot there for the
-- same reason: Pawl.Codec.TargetSlot is the only codec in the tree that writes one.
amountedSlots :: Value.Value -> Int
amountedSlots value = case value of
  Value.Array a -> sum (fmap amountedSlots (Array.unwrap a))
  Value.Object o ->
    let pairs = Object.unwrap o
        keyed k = [Pair.value pair | pair <- pairs, String.unwrap (Pair.name pair) == Text.pack k]
        here :: Int
        here = if null (keyed "pool") || null (keyed "amount") then 0 else 1
     in here + sum (fmap (amountedSlots . Pair.value) pairs)
  Value.String _ -> 0
  Value.Null _ -> 0
  Value.Boolean _ -> 0
  Value.Number _ -> 0
...
lintSpec s registry = Spec.describe s "Lint" $ do
  ...
  -- The same field's OTHER question: what may the bound itself say? Nothing that
  -- reads a slot. A target slot is matched at CR 601.2c -- CR 602.2b and CR 603.3d
  -- importing that rule for the other carriers -- and Pawl.Engine.Target.slotContext
  -- evaluates the bound against CR 113.7's SOURCE, which is the permanent or card
  -- the ability came from and not the announcement. Quantity.InSlot therefore finds
  -- no binding, the bound is Nothing, and ManaValueAtMostAmount is vacuously False:
  -- a slot whose only narrowing is that atom would admit nothing at all, and CR
  -- 603.3c would remove the ability from the stack for want of a legal target.
  -- Pawl.TargetSpec's "a bound that names a slot admits nothing at all" is the pair
  -- that shows it on a board; this is what keeps such a card out of the pool.
  --
  -- Not implemented: a bound read off the announcement's own bindings, which is
  -- what a printed "where X is the amount of damage this creature dealt to that
  -- player" wants and what would let this lint relax (gap #2666).
  --
  -- NOT folded into the reads-equal-declares equality instead
  -- (Pawl.Engine.Resolve.modeSlots): that would ACCEPT the shape wherever the
  -- carrier binds the name the bound reads -- which a triggered ability whose bound
  -- names its own CR 603.2 event amount does -- and that is precisely the case that
  -- compares against nothing.
  Spec.it s "CR 601.2c no card's computed bound reads a slot" $ do
    ps <- S.allPrintings s
    -- The REJECTING direction first, the pool having no offender to show it with:
    -- Celestine's own shape with the bound taken off a binding instead of off the
    -- board, which is the one thing that differs between the two slots.
    piker <- S.printingOf s registry "Goblin Piker"
    let slotWithAmount quantity =
          (S.combinedFace piker)
            { Face.spell =
                Modal.MkModal
                  ( Seq.singleton
                      ( Mode.MkMode
                          (Seq.singleton (Clause.MkClause Nothing Nothing Nothing Optionality.Mandatory Nothing Seq.empty))
                          (Map.singleton (SlotName.MkSlotName (Text.pack "target")) (TargetSlot.withAmount quantity (TargetSlot.required Pool.Creatures (Just Filter.Type.ManaValueAtMostAmount))))
                      )
                  )
                  (ModeSelection.ChooseExactly 1)
            }
        bound = slotWithAmount (Quantity.Type.InSlot (SlotName.MkSlotName (Text.pack "thatMuch")))
        tallied = slotWithAmount (Quantity.Type.LifeGainedThisTurn (PlayerRef.Relative PlayerRelation.You))
    Spec.assertBool s (amountOffends bound) "a bound naming a slot is an offence"
    Spec.assertEqWith s "and it is counted as one" (amountSlotCounts bound) (0, 1)
    Spec.assertBool s (not (amountOffends tallied)) "while the same slot's bound taken off the board is not"
    Spec.assertEqWith s "and that one is counted as inert" (amountSlotCounts tallied) (1, 0)
    Spec.assertEqWith s "no committed card writes a bound that reads a slot" (fmap (S.nameOf . Printing.card) (filter (anyFace amountOffends . Printing.card) ps)) []
    -- NOT vacuous: the pool authors the field, so the sweep above is iterating over
    -- real bounds rather than over nothing. The same two cards the case above names.
    Spec.assertEqWith
      s
      "and the pool's two committed bounds are both inert"
      (foldr (\p (i, r) -> let (i2, r2) = amountSlotCounts (S.combinedFace p) in (i + i2, r + r2)) (0, 0) ps)
      (2, 0)
