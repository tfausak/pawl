  ...
  Spec.describe s "Pawl.Engine.Mulligan" $ do
    ...
    Spec.it s "a hand action's effects can name their own card through the reserved self slot" $ do
      ...
      case Game.zoneMembers Zone.Hand S.alice drawn of
        ...
        oid : _ -> do
          let after = S.runPure S.identityAnswer drawn (Resolve.performHandAction oid S.alice [Effect.MoveToZone (MoveToZone.MkMoveToZone (ObjectRef.InSlot Binding.triggerSource) Zone.Exile EntryRiders.defaultValue Nothing Nothing LibraryPlacement.defaultValue Nothing)])
