...
spec s registry = Spec.describe s "Pawl.Engine.Event" $ do
  ...
  Spec.it s "CR 800.4b no token is created under the control of a player who has left the game" $ do
    ...
    let goblinCard = Printing.card piker
        ...
        after = S.runPure S.identityAnswer gone (Event.createTokens S.alice goblinCard Nothing 2 TapState.Untapped Map.empty Nothing)
  ...
  Spec.it s "CR 800.4b a player still in the game still gets their tokens" $ do
    ...
    let goblinCard = Printing.card piker
        gone = S.departs Departure.Type.Conceded S.alice S.threePlayerGame
        before = Game.objectCount gone
        after = S.runPure S.identityAnswer gone (Event.createTokens S.bob goblinCard Nothing 2 TapState.Untapped Map.empty Nothing)
  ...
  Spec.it s "CR 800.4d no card is conjured under a player who has left the game" $ do
    ...
    let goblinCard = Printing.card piker
        gone = S.departs Departure.Type.Conceded S.alice S.threePlayerGame
        before = Game.objectCount gone
        ontoBattlefield = S.runPure S.identityAnswer gone (Event.conjureOntoBattlefield S.alice goblinCard Nothing 2 TapState.Untapped)
        intoHand = S.runPure S.identityAnswer gone (Event.conjure S.alice goblinCard Nothing Zone.Hand LibraryPosition.defaultValue)
    Spec.assertBool s (notElem S.alice (Game.stillPlaying gone)) "alice really has left"
  ...
  Spec.it s "CR 800.4d a player still in the game still gets their conjured cards" $ do
    ...
    let goblinCard = Printing.card piker
        gone = S.departs Departure.Type.Conceded S.alice S.threePlayerGame
        before = Game.objectCount gone
        ontoBattlefield = S.runPure S.identityAnswer gone (Event.conjureOntoBattlefield S.bob goblinCard Nothing 2 TapState.Untapped)
        intoHand = S.runPure S.identityAnswer gone (Event.conjure S.bob goblinCard Nothing Zone.Hand LibraryPosition.defaultValue)
  ...
  Spec.it s "CR 111.2 createTokens puts a token on the battlefield and emits an enters event" $ do
    ...
    let base = Setup.emptyGame S.bothPlayers
        ...
        after = S.runPure S.identityAnswer base (Event.createTokens S.alice goblinCard Nothing 1 TapState.Untapped Map.empty Nothing)
  ...
  Spec.it s "CR 603.2 SelfDealsCombatDamageToPlayer matches the bearer's combat damage to a player" $ do
    let bearer = ObjectId.MkObjectId 1
        ev = GameEvent.DamageDealt (DamageEvent.MkDamageEvent bearer (Recipient.ToPlayer S.bob) 2 False False False 0 Nothing Nothing mempty False DamageKind.Combat)
  ...
  Spec.it s "it does not match combat damage to a creature" $ do
    let bearer = ObjectId.MkObjectId 1
        ev = GameEvent.DamageDealt (DamageEvent.MkDamageEvent bearer (Recipient.ToCreature (ObjectId.MkObjectId 2)) 2 False False False 0 Nothing Nothing mempty False DamageKind.Combat)
