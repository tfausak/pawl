...
nonexistenceSpec :: (Monad m, Monad n) => Spec.Spec m n -> Registry.Registry m -> n ()
nonexistenceSpec s registry = Spec.describe s "Nonexistence" $ do
  ...
  -- CR 702.26b at the CR 613.1b layer-2 control fold. Rule 702.26e is NOT the
  -- citation: it governs a continuous effect generated by the RESOLUTION of a
  -- spell or ability, and this grant is a static ability, whose set CR 611.3a
  -- re-derives at every moment rather than freezing. So it is rule 702.26b's
  -- own "treated as though it does not exist" that has to answer here.
  --
  -- Synthetic Puppeteer's Yoke ("You control all permanents enchanted player
  -- controls") is the pool's only static control grant reached through a PLAYER,
  -- so its affected set is the one that would name a permanent by its controller
  -- rather than by walking a zone -- except that
  -- Pawl.Engine.Projection.View.controlNames bounds it by the battlefield, which
  -- Phasing.phaseOut has already removed the phased-out permanent from.
  --
  -- One board, two of bob's Pikers, differing only in which one alice's Reality
  -- Ripple was aimed at.
  Spec.it s "CR 702.26b a phased-out permanent leaves a control grant reached through its controller" $ do
    island <- S.printingOf s registry "Island"
    piker <- S.printingOf s registry "Goblin Piker"
    ripple <- S.printingOf s registry "Reality Ripple"
    ...
    let ((victim, bystander), spell, board) =
          rippleBoard island ripple $ \gs ->
            let (a, g1) = S.addPermanent piker S.bob gs
                (b, g2) = S.addPermanent piker S.bob g1
                (aura, g3) = S.addPermanent yoke S.alice g2
             in ((a, b), S.attachTo aura (Recipient.ToPlayer S.bob) g3)
        after = rippleAt victim spell board
        back = untapStep S.alice after
    Spec.assertEqWith s "setup: the Yoke had taken both of bob's Pikers" (fmap (\oid -> Projection.controllerOf oid board) [victim, bystander]) [Just S.alice, Just S.alice]
    Spec.assertEqWith s "phased out, the Yoke cannot reach it and bob has it back" (Projection.controllerOf victim after) (Just S.bob)
    Spec.assertEqWith s "while the Piker still on the battlefield stays alice's" (Projection.controllerOf bystander after) (Just S.alice)
    -- CR 702.26c: it exists again, so the set names it again -- CR 611.3a's live
    -- half, which a frozen set could not do. It phased out under alice, whose
    -- untap step is therefore the one that brings it back (CR 702.26a).
    Spec.assertEqWith s "and it comes back under the Yoke's controller" (Projection.controllerOf victim back) (Just S.alice)
