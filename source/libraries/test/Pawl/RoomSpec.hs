...
lockAbility keys = case drop 1 (Face.activatedAbilities (S.combinedFace keys)) of
  ...
  [] -> ActivatedAbility.MkActivatedAbility (Cost.MkCost Nothing []) [] (Modal.MkModal Seq.empty (ModeSelection.ChooseExactly 1)) [] Activator.Controller Nothing Nothing Nothing
