...
setupSpec s registry = Spec.describe s "Setup" $ do
  ...
  Spec.it s "CR 903.7 a Commander deck starts its player at 40 life" $ do
    ...
    let build commander =
          S.runPure S.identityAnswer (Setup.emptyGame S.bothPlayers) $
            Setup.createDeck S.alice Deck.MkDeck {Deck.cards = Map.empty, Deck.commander = commander, Deck.vanguard = Nothing, Deck.dungeons = Set.empty, Deck.sideboard = Map.empty, Deck.conspiracies = Map.empty, Deck.attractions = Map.empty}
  ...
  Spec.it s "CR 903.12f a Brawl game starts its players at 25 at two seats and 30 at three" $ do
    ...
    let build seats brawl =
          S.runPure S.identityAnswer (settingsOf brawl (Setup.emptyGame seats)) $
            Setup.createDeck S.alice Deck.MkDeck {Deck.cards = Map.empty, Deck.commander = Set.singleton shimatsu, Deck.vanguard = Nothing, Deck.dungeons = Set.empty, Deck.sideboard = Map.empty, Deck.conspiracies = Map.empty, Deck.attractions = Map.empty}
  ...
  Spec.it s "CR 727.1 / 729.2 a rebuilt Brawl game is still a Brawl game" $ do
    shimatsu <- S.printingOf s registry "Shimatsu the Bloodcloaked"
    mountain <- S.printingOf s registry "Mountain"
    let deck = Deck.MkDeck {Deck.cards = Map.singleton mountain 10, Deck.commander = Set.singleton shimatsu, Deck.vanguard = Nothing, Deck.dungeons = Set.empty, Deck.sideboard = Map.empty, Deck.conspiracies = Map.empty, Deck.attractions = Map.empty}
        built brawl =
...
restartSpec s registry = Spec.describe s "restart (CR 727)" $ do
  ...
  Spec.it s "CR 903.6/903.9c a restarted Commander game puts the melded commander's own card into the command zone and its partner into the deck" $ do
    ...
    let (meldedId, board) = meldedBoard (Setup.emptyGame S.bothPlayers) battlements garrison mountain
        components = componentsOn meldedId board
        designating printingId gs = gs {GameState.players = Map.adjust (\p -> p {Player.commander = Set.singleton printingId}) S.alice (GameState.players gs)}
...
subgameSpec s registry = Spec.describe s "subgames (CR 729)" $ do
  ...
  Spec.it s "CR 729.4a a phased-in permanent a subgame takes triggers its leaves-the-battlefield ability" $ do
    ...
    let (tusk, g1) = S.addPermanent thragtusk S.alice S.threePlayerGame
        ...
        sub0 = Setup.subgameStateFrom S.alice parent
        (_, crossedSub) = Event.bringInFrom OutsideDestination.Hand S.alice tusk sub0
  ...
  Spec.it s "CR 702.26b a phased-out Titania's Song a subgame takes hands nothing over" $ do
    ...
    let (songId, g1) = S.addPermanent titaniasSong S.alice (Setup.emptyGame S.bothPlayers)
        ...
        crossFrom parent = Setup.applyCrossings (snd (Event.bringInFrom OutsideDestination.Hand S.alice songId (Setup.subgameStateFrom S.alice parent))) parent
  ...
  Spec.it s "CR 608.2h: the second crossing's last known information is read after the first has gone" $ do
    ...
    let g0 = Setup.emptyGame S.bothPlayers
        ...
        sub0 = Setup.subgameStateFrom S.alice parent
        (_, crossed1) = Event.bringInFrom OutsideDestination.Hand S.alice anthemId sub0
        (_, crossedSub) = Event.bringInFrom OutsideDestination.Hand S.alice elfId crossed1
        after = Setup.applyCrossings crossedSub parent
  ...
  Spec.it s "CR 400.11b/729.5: the pool a subgame's wish spent comes back spent, and nothing else does" $ do
    ...
    let g0 = Setup.emptyGame S.bothPlayers
        ...
        (_, spentSub) = Event.bringIn OutsideDestination.Hand S.alice printingId sub0
  ...
  Spec.it s "CR 400.11b/800.4a a player who departs inside the subgame gets their spend undone" $ do
    ...
    let g0 = Setup.emptyGame S.threePlayers
        ...
        sub0 = Setup.subgameStateFrom S.alice parent
        (_, aliceSpent) = Event.bringIn OutsideDestination.Hand S.alice printingId sub0
        (_, bobSpent) = Event.bringIn OutsideDestination.Hand S.bob printingId aliceSpent
  ...
  Spec.it s "CR 729.4a/800.4a a main-game card a departed player wished in is in neither game after" $ do
    ...
    let g0 = Setup.emptyGame S.threePlayers
        ...
        sub0 = Setup.subgameStateFrom S.alice parent
        (minted, crossedSub) = Event.bringInFrom OutsideDestination.Hand S.bob bobsId sub0
