...
-- `targetingSpell` narrowed to ONE target, with that target's own view filled
-- beside the recipient the way Pawl.Engine.Projection.View.viewOfCharacteristics
-- fills it -- which is what TargetsOnlyOne's nest is matched against.
aimedAt :: Recipient.Recipient -> Filter.View -> Filter.View
aimedAt r target =
  targetingSpell
    { Filter.targets = Set.singleton r,
      Filter.targetViews = Map.singleton r target
    }
...
spec s = Spec.describe s "Pawl.Engine.Filter" $ do
  ...
  Spec.describe s "TargetsOnlyOne" $ do
    Spec.it s "matches one target the nest describes, and nothing else" $ do
      ...
      Spec.assertBool s (not (Filter.matches self (aimedAt (Recipient.ToCreature (ObjectId.MkObjectId 9)) blackCreature) (Filter.Type.TargetsOnlyOne (Filter.Type.HasCardType CardType.Land)))) "which is not a land"
      -- CR 115.1a / 110.1: the SAME object reached through CR 110.1's permanent
      -- pool rather than CR 115.1a's creature pool. The nest reads the target's
      -- own view, so the tag the spell's slot carries no longer narrows it --
      -- the reading that had pawl running stricter than printed, see #3272.
      Spec.assertBool s (Filter.matches self (aimedAt (Recipient.ToObject (ObjectId.MkObjectId 9)) blackCreature) (Filter.Type.TargetsOnlyOne (Filter.Type.HasCardType CardType.Creature))) "and so is the same creature named by CR 110.1's permanent pool"

    Spec.it s "CR 115.1 judges the nest in the candidate's own context" $ do
      Spec.assertBool s (Filter.matches self (aimedAt (Recipient.ToCreature (ObjectId.MkObjectId 9)) blackCreature) (Filter.Type.TargetsOnlyOne (Filter.Type.ControlledBy PlayerRelation.You))) "player 0 controls the one target, from player 0"
      Spec.assertBool s (not (Filter.matches other (aimedAt (Recipient.ToCreature (ObjectId.MkObjectId 9)) blackCreature) (Filter.Type.TargetsOnlyOne (Filter.Type.ControlledBy PlayerRelation.You)))) "and player 1 does not, from player 1"

    Spec.it s "CR 115.1 a player target answers a player view" $ do
      Spec.assertBool s (Filter.matches self (aimedAt (Recipient.ToPlayer (PlayerId.MkPlayerId 1)) aPlayer) (Filter.Type.TargetsOnlyOne (Filter.Type.IsPlayer PlayerRelation.AnyPlayer))) "one player target"
      Spec.assertBool s (not (Filter.matches self (aimedAt (Recipient.ToPlayer (PlayerId.MkPlayerId 1)) aPlayer) (Filter.Type.TargetsOnlyOne (Filter.Type.HasCardType CardType.Creature)))) "which is no creature"

    Spec.it s "asks arity as well as the description" $ do
      Spec.assertBool s (not (Filter.matches self (targetingSpell {Filter.targets = Set.fromList [Recipient.ToCreature (ObjectId.MkObjectId 9), Recipient.ToCreature (ObjectId.MkObjectId 10)], Filter.targetViews = Map.fromList [(Recipient.ToCreature (ObjectId.MkObjectId 9), blackCreature), (Recipient.ToCreature (ObjectId.MkObjectId 10), blackCreature)]}) (Filter.Type.TargetsOnlyOne (Filter.Type.HasCardType CardType.Creature)))) "two creature targets are not a single one"
      Spec.assertBool s (not (Filter.matches self blackCreature (Filter.Type.TargetsOnlyOne (Filter.Type.HasCardType CardType.Creature)))) "and a permanent targets nothing"
      Spec.assertBool s (not (Filter.matches self aPlayer (Filter.Type.TargetsOnlyOne (Filter.Type.IsPlayer PlayerRelation.AnyPlayer)))) "as does a player"

    -- CR 608.2b: a target whose object has left leaves the map without the key,
    -- and no description is answerable of it.
    Spec.it s "CR 608.2b a target with no view answers nothing" $
      Spec.assertBool s (not (Filter.matches self (targetingSpell {Filter.targets = Set.singleton (Recipient.ToCreature (ObjectId.MkObjectId 9)), Filter.targetViews = Map.empty}) (Filter.Type.TargetsOnlyOne (Filter.Type.And [])))) "not even the trivial one"
  ...
  Spec.describe s "IsAttackingPlayer" $ do
    ...
    let attackingPlayer pid = blackCreature {Filter.attacking = True, Filter.attackingPlayer = Just (PlayerId.MkPlayerId pid)}
        -- CR 508.1b's SECOND subject, which viewOfCharacteristics fills the other
        -- field for: attacking, attacking no player, and attacking a planeswalker
        -- the perspective controls.
        attackingPlaneswalker =
          blackCreature
            { Filter.attacking = True,
              Filter.attackingPlayer = Nothing,
              Filter.attackingPlaneswalkerController = Just (PlayerId.MkPlayerId 0)
            }
  ...
  Spec.describe s "IsAttackingPlaneswalker" $ do
    ...
    let attackingPlaneswalkerOf pid =
          blackCreature
            { Filter.attacking = True,
              Filter.attackingPlaneswalkerController = Just (PlayerId.MkPlayerId pid)
            }
        attackingPlayer pid = blackCreature {Filter.attacking = True, Filter.attackingPlayer = Just (PlayerId.MkPlayerId pid)}

    Spec.it s "matches a creature attacking a planeswalker the perspective controls" $ do
    ...
    Spec.it s "reads the relation against the perspective" $ do
      Spec.assertBool s (not (Filter.matches self (attackingPlaneswalkerOf 1) (Filter.Type.IsAttackingPlaneswalker PlayerRelation.You))) "someone else's planeswalker is not yours"
      Spec.assertBool s (Filter.matches self (attackingPlaneswalkerOf 1) (Filter.Type.IsAttackingPlaneswalker PlayerRelation.Opponent)) "it is an opponent's"
      Spec.assertBool s (Filter.matches self (attackingPlaneswalkerOf 1) Filter.Type.IsAttacking) "and it is attacking either way"

    -- The other half of CR 509.1a's split, from this atom's side: the two subjects
    -- are disjoint, so a creature attacking a PLAYER is not attacking a
    -- planeswalker however squarely that player is its defending player (CR
    -- 508.5).
    Spec.it s "does not match a creature attacking a player" $ do
      Spec.assertBool s (not (Filter.matches self (attackingPlayer 0) (Filter.Type.IsAttackingPlaneswalker PlayerRelation.You))) "attacking you is not attacking your planeswalker"
      Spec.assertBool s (not (Filter.matches self (attackingPlayer 0) (Filter.Type.IsAttackingPlaneswalker PlayerRelation.AnyPlayer))) "nor anyone's"

    -- IsAttackingPlayer's posture, and for its reason: CR 109.5's "you" is what
    -- the match is framed by, so AnyPlayer demands one too.
    Spec.it s "is vacuously false with no perspective" $ do
      Spec.assertBool s (not (Filter.matches noPerspective (attackingPlaneswalkerOf 0) (Filter.Type.IsAttackingPlaneswalker PlayerRelation.AnyPlayer))) "no perspective"

    -- CR 506.4c: a creature whose planeswalker left combat stays an attacking
    -- creature attacking nothing, which is the view with both fields Nothing.
    Spec.it s "does not match a creature attacking nothing at all" $ do
      Spec.assertBool s (not (Filter.matches self (blackCreature {Filter.attacking = True}) (Filter.Type.IsAttackingPlaneswalker PlayerRelation.AnyPlayer))) "attacking nothing"
      Spec.assertBool s (not (Filter.matches self blackCreature (Filter.Type.IsAttackingPlaneswalker PlayerRelation.AnyPlayer))) "not attacking"

    -- CR 506.3: a player attacks nothing, so Pawl.Engine.Filter.playerView leaves
    -- the field Nothing, as it does for IsAttackingPlayer.
    Spec.it s "a player candidate is vacuously false" $ do
      Spec.assertBool s (not (Filter.matches self aPlayer (Filter.Type.IsAttackingPlaneswalker PlayerRelation.AnyPlayer))) "player"

  Spec.describe s "IsAttackingBattle" $ do
    -- CR 508.1b's third subject: attacking, attacking no PLAYER and no
    -- PLANESWALKER, and the battle it is attacking is protected by `pid`.
    -- Pawl.Engine.Projection fills the three fields off the same map's three arms,
    -- so no two of them are ever Just at once.
    let attackingBattleOf pid =
          blackCreature
            { Filter.attacking = True,
              Filter.attackingBattleProtector = Just (PlayerId.MkPlayerId pid)
            }
        attackingPlayer pid = blackCreature {Filter.attacking = True, Filter.attackingPlayer = Just (PlayerId.MkPlayerId pid)}
        attackingPlaneswalkerOf pid = blackCreature {Filter.attacking = True, Filter.attackingPlaneswalkerController = Just (PlayerId.MkPlayerId pid)}

    Spec.it s "matches a creature attacking a battle the perspective protects" $ do
      Spec.assertBool s (Filter.matches self (attackingBattleOf 0) (Filter.Type.IsAttackingBattle PlayerRelation.You)) "attacking your battle"

    -- CR 310.9d is what makes the seat compared the PROTECTOR: the same attacking
    -- creature answers one way for the protector and the other for anybody else,
    -- the battle's own controller included. `self` is player 0.
    Spec.it s "reads the relation against the perspective" $ do
      Spec.assertBool s (not (Filter.matches self (attackingBattleOf 1) (Filter.Type.IsAttackingBattle PlayerRelation.You))) "a battle someone else protects is not yours"
      Spec.assertBool s (Filter.matches self (attackingBattleOf 1) (Filter.Type.IsAttackingBattle PlayerRelation.Opponent)) "it is an opponent's"
      Spec.assertBool s (Filter.matches self (attackingBattleOf 1) Filter.Type.IsAttacking) "and it is attacking either way"

    -- CR 509.1a's three subjects are disjoint, from this atom's side: neither of
    -- the other two arms of AttackTarget answers this one.
    Spec.it s "does not match a creature attacking a player or a planeswalker" $ do
      Spec.assertBool s (not (Filter.matches self (attackingPlayer 0) (Filter.Type.IsAttackingBattle PlayerRelation.AnyPlayer))) "attacking you is not attacking your battle"
      Spec.assertBool s (not (Filter.matches self (attackingPlaneswalkerOf 0) (Filter.Type.IsAttackingBattle PlayerRelation.AnyPlayer))) "nor is attacking your planeswalker"

    -- IsAttackingPlayer's posture, and for its reason: CR 109.5's "you" is what
    -- the match is framed by, so AnyPlayer demands one too.
    Spec.it s "is vacuously false with no perspective" $ do
      Spec.assertBool s (not (Filter.matches noPerspective (attackingBattleOf 0) (Filter.Type.IsAttackingBattle PlayerRelation.AnyPlayer))) "no perspective"

    -- CR 506.4c for a battle, and CR 310.11's mid-repair designation: both leave
    -- the field Nothing, and an attacking creature with nothing to read is not
    -- attacking a battle anybody protects.
    Spec.it s "does not match a creature attacking nothing at all" $ do
      Spec.assertBool s (not (Filter.matches self (blackCreature {Filter.attacking = True}) (Filter.Type.IsAttackingBattle PlayerRelation.AnyPlayer))) "attacking nothing"
      Spec.assertBool s (not (Filter.matches self blackCreature (Filter.Type.IsAttackingBattle PlayerRelation.AnyPlayer))) "not attacking"

    -- CR 506.3: a player attacks nothing, so Pawl.Engine.Filter.playerView leaves
    -- the field Nothing, as it does for the two atoms above.
    Spec.it s "a player candidate is vacuously false" $ do
      Spec.assertBool s (not (Filter.matches self aPlayer (Filter.Type.IsAttackingBattle PlayerRelation.AnyPlayer))) "player"
