...
representativeEvents cond =
  let departed = representativeDeparted
   ...
   in case cond of
        ...
        -- TWO of them, for SelfIsDealtDamage's reason below: this arm admits both
        -- of CR 120.3's damage kinds, so a floor claimed for combat alone would
        -- come apart on the noncombat one.
