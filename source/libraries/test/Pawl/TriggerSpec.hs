  ...
  Spec.describe s "Scan" $ do
    ...
    Spec.it s "CR 603.2b the active player rides a step trigger in the reserved slot" $ do
      let upkeepOf pid = GameEvent.StepBegan (StepBegan.MkStepBegan (Phase.Beginning BeginningStep.Upkeep) pid)
          ...
          boundOf pid = Binding.targetsOf (Event.eventBindings (Setup.emptyGame S.bothPlayers) Nothing cond (upkeepOf pid))
      ...
      Spec.assertEqWith
        ...
        (Binding.targetsOf (Event.eventBindings (Setup.emptyGame S.bothPlayers) Nothing (TriggerCondition.StepBegins (StepBegins.MkStepBegins (Phase.Beginning BeginningStep.Upkeep) TurnScope.ControllersTurn)) (upkeepOf S.alice)))
