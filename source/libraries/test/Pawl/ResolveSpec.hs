...
resolveSpec s registry = Spec.describe s "Resolve" $ do
  ...
  Spec.it s "CR 106.4 slotsOf finds the slot an AddMana recipient names" $ do
    ...
    Spec.assertEqWith s "a named recipient is a read" (Resolve.slotsOf (Effect.AddMana (ManaAddition.MkManaAddition (PlayerRef.InSlot slot) ManaProduction.AnyColor 1 ManaRetention.Ordinary Nothing Nothing))) (Map.singleton slot SlotArity.One)
    Spec.assertEqWith s "and CR 109.5's unwritten one names no slot" (Resolve.slotsOf (Effect.AddMana (ManaAddition.MkManaAddition (PlayerRef.Relative PlayerRelation.You) ManaProduction.AnyColor 1 ManaRetention.Ordinary Nothing Nothing))) Map.empty
  ...
  Spec.it s "CR 605 manaProduced reads AddMana whole, and nothing else" $ do
    let plain = ManaAddition.MkManaAddition (PlayerRef.Relative PlayerRelation.You) (ManaProduction.OfType (ManaType.Colored Color.Green)) 1 ManaRetention.Ordinary Nothing Nothing
        anyColor = ManaAddition.MkManaAddition (PlayerRef.Relative PlayerRelation.You) ManaProduction.AnyColor 1 ManaRetention.Ordinary Nothing Nothing
    ...
    let restricted = ManaAddition.MkManaAddition (PlayerRef.Relative PlayerRelation.You) ManaProduction.AnyColor 1 ManaRetention.Ordinary (Just (ManaRestriction.onlyCasts (Filter.Type.HasCardType CardType.Artifact))) Nothing
  ...
  Spec.it s "CR 701.23 Search fetches a basic land to the battlefield tapped" $ do
    ...
    let base = Setup.emptyGame S.bothPlayers
        ...
        abilObj =
          Object.MkObject S.alice Nothing (Source.OfAbility ActivatedAbilitySource.MkActivatedAbilitySource {ActivatedAbilitySource.source = ObjectId.MkObjectId 0, ActivatedAbilitySource.ability = ability}) Zone.Stack TapState.Untapped Facing.FaceUp False False 0 (Sickness.Settled S.alice) (Binding.fromChoices Map.empty Nothing (Seq.singleton (ModeIndex.MkModeIndex 0))) Map.empty Map.empty Nothing Nothing Nothing Set.empty Nothing ts Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Set.empty Set.empty Map.empty Map.empty False False False False 0 (Mana.MkMana []) Nothing Nothing Set.empty Set.empty False Set.empty Set.empty
  ...
  -- twice, differing in exactly one answer, CR 603.5's "may".
  --
  -- Three clauses rather than two, which is the point: the draw is its own
  -- clause naming the "may" clause's ordinal (Clause.ifTaken), so the decline
  -- must skip the draw and leave the damage alone. Baral, Chief of Compliance's
  -- "you may draw a card. If you do, discard a card" needs none of that, since
  -- CR 121.3 makes its first action happen whenever it is chosen and one
  -- optional clause carrying both instructions says the same thing.
  Spec.it s "CR 608.2c Tweeze's \"If you do\" draws when the discard was taken" $ do
    board <- tweezeBoard s registry
    let after = tweezeResolved OptionalDecision.Exercises board
        nameOf = Just . CardName.MkCardName . Text.pack
    Spec.assertEqWith s "CR 608.2c the gated clause ran: the drawn Piker is in alice's hand" (List.sort (namesIn Zone.Hand S.alice after)) (List.sort [nameOf "Goblin Piker", nameOf "Chaos Charm"])
  ...
  Spec.it s "CR 615 Fog prevents combat damage but not spell damage (the gate)" $ do
    ...
    let base = S.landsInPlay forest 1
        (victim, gs0) = S.addPermanent piker S.bob base
...
bifurcateBoard s registry = do
  ...
  let (pikerId, g1) = S.addPermanent piker S.alice (S.landsInPlay forest 4)
...
-- 104.3c cannot decide the game first. Bob is the damage's target, and the two
-- cards in hand and the ones in the library are three different names, so no
-- assertion below can pass by coincidence.
tweezeBoard :: (Monad m) => Spec.Spec m n -> Registry.Registry m -> m (GameState.GameState, ObjectId.ObjectId, ObjectId.ObjectId)
tweezeBoard s registry = do
  mountain <- S.printingOf s registry "Mountain"
  ...
  let (base, tweezeId) = S.handOne tweeze (S.landsInPlay mountain 3)
      (maidenId, withMaiden) = S.addHandCard maiden S.alice base
...
tweezeResolved :: OptionalDecision.OptionalDecision -> (GameState.GameState, ObjectId.ObjectId, ObjectId.ObjectId) -> GameState.GameState
tweezeResolved decision (gs, tweezeId, maidenId) =
  S.runPure (tweezeAnswer decision maidenId) gs (S.cast S.alice tweezeId >> Stack.resolveTop)
...
withArbiter s registry board = do
  ...
  pure board {moogleState = snd (S.addPermanent arbiter S.bob (moogleState board))}
