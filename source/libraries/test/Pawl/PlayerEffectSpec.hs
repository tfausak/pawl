...
runedHaloSpec s registry =
  Spec.describe s "RunedHalo" $ do
    ...
    -- CR 702.16c's second sentence, and the clause #2387 was open on: "such
    ...
    Spec.it s "CR 702.16c / 704.5m an Aura already enchanting the player is buried once she gains protection from its name" $ do
      ...
      Spec.assertEqWith s "in its OWNER's graveyard -- bob cast it, so it is his" (length (Game.zoneMembers Zone.Graveyard S.bob named)) 1
