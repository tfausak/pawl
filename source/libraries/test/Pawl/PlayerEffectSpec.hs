...
-- protection from a CHOSEN NAME, so it carries rule 702.16b's and rule 702.16c's
-- player halves in with it (Pawl.Engine.Target.targetable and
-- Pawl.Engine.Sba.fallsOff). The Stasis Coffin is the other quality shape, one
-- the card states. Curse of Vitality is the enchant-player Aura on the other side
-- of both.
--
