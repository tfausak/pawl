    ...
    { TriggeredAbility.condition = TriggerCondition.OpponentLostLifeDuringYourTurn,
      TriggeredAbility.modal =
        Modal.MkModal
          ( Seq.singleton
              ( Mode.MkMode
                  (Seq.singleton (Clause.MkClause Nothing Nothing Optionality.Mandatory Nothing (Seq.singleton (Effect.IncreaseSpeed (PlayerQuantity.MkPlayerQuantity (PlayerRef.Relative PlayerRelation.You) (Quantity.Literal 1))))))
                  Map.empty
              )
          )
