...
-- Pawl.Engine.PlayerEffect, on the other axis CR 613.11 reaches: that module answers
-- rules questions about a PLAYER, this one answers the one rules question CR
-- 509.1c asks about a pair of CREATURES. Neither is a layer, and Pawl.Engine.Projection
-- sees neither -- CR 613.11 applies both "after all other continuous effects have
-- been applied".
--
-- This module is the only reader of Pawl.Types.BlockRequirement. Pawl.Engine.Combat asks
-- for requirement INSTANCES -- bare (blocker, attacker) pairs -- and never learns
-- which card produced one, the same posture Pawl.Engine.Cast takes toward
...
-- learns the restrictions and Pawl.Engine.Combat stays the only home for them. Pruning
...
--
-- `candidates` is CR 509.1a's chosen-from set (Pawl.Engine.Combat.legalBlockers) and
...
instances able candidates attackers gs =
  let -- Hoisted out of the walk exactly as Pawl.Engine.PlayerEffect.applying hoists them,
      ...
      fromPermanent source = case Game.cardOf source gs of
        ...
        Just card -> case Card.blockRequirements card of
          ...
          [] -> []
          requirements ->
            -- TWO ability losses, the same pair Pawl.Engine.PlayerEffect.applying asks
