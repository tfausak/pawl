...
-- out of another. A subset spanning two zones is satisfied whenever each zone's
-- part is, so grouping first loses no case and saves the enumeration.
--
...
satisfiable claims = all (all fits . List.subsequences) (byZone claims)
  where
...
repeats claims = case concatMap (Maybe.mapMaybe limit . List.subsequences) (byZone claims) of
  [] -> 1
...
-- times contends for. Exact rather than an approximation: `byZone` adds the
...
-- The claims grouped into the pools they draw on: one list per zone, and within
...
byZone :: [Claim] -> [[(Set.Set ObjectId, Natural)]]
byZone claims =
  fmap Map.toList
    . Map.elems
    $ Map.fromListWith
      (Map.unionWith (+))
      (fmap (\claim -> (Claim.zone claim, Map.singleton (Claim.pool claim) (Claim.count claim))) claims)
