...
-- in the DamageRewrite and the use count -- CR 615.7's countdown, CR 615.1's
-- unbounded shield, that same shield with CR 614.3's single use beside it (CR
-- 615.8), or CR 614.9's redirection.
--
...
-- pairs CR 609.7b's properties with CR 615.7's countdown, CR 615.8's single
        ...
        active =
          ActiveReplacement.MkActiveReplacement
            { ActiveReplacement.effect = re,
              ...
              -- the use count is not what ends it (see Pawl.Types.Uses) and both
              -- countdown callers pass Unlimited. CR 615.8's shield is the one
              -- that passes Once: "once an instance of damage from that source
              -- has been prevented, any subsequent instances ... are dealt
              -- normally" counts APPLICATIONS, which is what this field is.
...
applyOneEffect runSubgame resolving source controller legal chosen effect = case effect of
  ...
  Effect.RollDie rollDie -> do
    ...
    let perDie :: Game Integer
        perDie = do
          ...
          -- count above one adds it to each. CR 107.2's posture for a modifier
          -- that cannot be evaluated: no modifier at all.
  ...
  -- instructions.
  --
  -- THREE WRITERS, three roads. Pawl.Engine.Event's ChoiceByCoinFlip arm is the
  -- flip made as a permanent enters (Molten Sentry) and Pawl.Engine.Cost's
  -- CostComponent.FlipCoin arm is the flip a COST names (Karplusan Minotaur),
  -- each recording its own CoinFlipped. Every road that flips records, which is
  -- what keeps this event the log of CR 705.1 flips rather than of one opcode.
  --
  ...
  Effect.FlipCoin flipCoin -> do
    ...
    let flipOnce acc _ = do
          hit <- case FlipCoin.reading flipCoin of
            -- CR 705.2's win/lose flip, whole: the call, the flip, the
            -- comparison and the CR 705.1 event all belong to
            -- Event.flipWinLoseCoin, which is also the road a COST'S flip takes.
            -- That module's haddock argues the order of the two questions, and
            -- Pawl.CoinSpec's "only the flipping player calls" proves it by what
            -- the engine ASKED rather than by anything the board shows.
            CoinReading.Wins -> Event.flipWinLoseCoin controller statements
            ...
            -- against TriggerCondition's two coin-flip conditions, neither of
            -- which matches an outcome of Nothing; see
  ...
  -- tied for most votes at one slot (Council's Judgment's exile), or one tally
  -- per word for the clause each word is connected to (Plea for Power).
  --
  -- An object vote's listed choices are swept ONCE, off the board as the vote
  -- begins, so every voter chooses from the same list -- rule 701.38a votes "for
  -- one choice from a list", singular. Through battlefieldMatching, so the sweep
  -- reads the projection and a permanent that is a copy of something is judged
  -- by the characteristics it copied.
  --
  -- Nothing on the board changes while the seats vote, so the decider, the
  -- candidate list and rule 701.38d's ballot counts are all read off this one
  -- `gs`.
  Effect.Vote (Vote.MkVote starter choices) -> do
    gs <- State.get
    let -- Rule 701.38a's "specified player". A reference naming several names the
        ...
        -- reminder text has the votes go to different choices or the same one, so
        -- nothing lets the engine answer the second from the first.
        ballotsOf ask voter = Monad.replicateM (Natural.toIntSaturating (PlayerEffect.votesAllowed voter gs)) (ask voter)
        cast ask = fmap concat (Monad.forM voters (ballotsOf ask))
    case choices of
      VoteChoices.Objects objects -> do
        let candidates = battlefieldMatching legal resolving controller source gs (VoteObjects.filter objects)
        case candidates of
          ...
          first : rest -> do
            ...
            State.modify' (bindObjectsSlot resolving (VoteObjects.slot objects) (Seq.fromList winners))
      -- CR 701.38b's words. No sweep and no empty case: the listed choices are
      -- printed on the card, so the list cannot come up empty the way a
      ...
      VoteChoices.Words voteWords -> do
        ...
        --
        -- Every word, including the ones nobody voted for: Plea for Power's "if
        -- knowledge gets more votes or the vote is tied" compares the two tallies,
        -- and an absent slot would read as no answer rather than as zero.
        --
  ...
  Effect.PreventNextDamageInstance (PreventNextDamageInstance.MkPreventNextDamageInstance duration ref sourceFilter) -> do
    ...
    -- what makes it the NEXT instance rather than every one is Uses.Once, CR
    -- 614.3's use count, the same spelling CR 701.19a's "the next time this
    -- permanent would be destroyed this turn" already takes.
    --
    -- CR 609.7b falls out of that reading rather than needing an arm: a use is
    -- spent by Event.apply, which runs only where the shield actually prevented
    ...
    Monad.unless (null recipients) $ do
      ...
      sourceChoice <- chooseDamageSource controller resolving context gs (Just sourceFilter)
      case sourceChoice of
        -- CR 609.7a's pool was empty, so there is nothing to shield and no row is
...
-- | CR 706.1-706.6: throw `named` dice with `sides` sides for `controller`,
-- through every modification rule 706 names -- CR 614.1a's replacements, CR
-- 706.2b's rerolls and adjustments, CR 706.6's ignored rolls -- and answer each
-- kept die's final result beside the player who threw its final natural result.
...
recordRoll controller throwers results = do
  ...
  -- natural result: Pippa, Duchess of Dice's ruling gives a reroll of another
  -- player's die to the rerolling player's "whenever you roll" triggers.
...
throwDice controller sides named perDie = do
  let -- CR 706.1a's outcomes "numbered from 1 to N", BOTH ends included: the
      ...
      -- one printed modifier carrying a mana cost, an Attraction opener -- will
      -- not either: its roller IS its payer, "you may pay
      ...
      rollOne = do
        ...
        -- considered first, and what they leave is the natural result the
        -- instruction's own modifier adds to.
        modifier <- perDie
