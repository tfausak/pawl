    ...
    { -- CR 500.1 / 505.1a: a turn has exactly one precombat main phase and it is
      ...
      TriggeredAbility.modal =
        Modal.MkModal
          ( Seq.singleton
              ( Mode.MkMode
                  ( Seq.singleton . Clause.MkClause Nothing Nothing Optionality.Mandatory Nothing . Seq.fromList $
...
inherentPending events gs =
  let you = GameState.activePlayer gs
   ...
   in [ PendingTrigger.MkPendingTrigger TriggerSource.Sourceless you ability Map.empty Nothing
      | irradiated,
