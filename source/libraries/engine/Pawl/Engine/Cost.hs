  ...
  Just obj -> case Object.source obj of
    Source.OfCard printing ->
      let card = Printing.card printing
          ...
          -- applies. The increases and reductions are Pawl.Engine.Cost.total's job,
...
-- candidate -- the shape CR 702.42a's entwine needs. Pawl.Engine.Cast applies it to
...
-- that spell or ability (see rule 601.2b)." The seam Pawl.Engine.Cast and Pawl.Engine.Activate
...
-- this function's to compute, because the two callers genuinely differ. Pawl.Engine.Cast
...
-- summoning-sickness gate? The CLASSIFICATION Pawl.Engine.Activate reads, so that this
...
payComponent pid oid component = case component of
  ...
  -- to Ghitu Fire-Eater. Pawl.Engine.Event's scan reads that zone -- see its cycledCard
  -- candidate source.
  --
