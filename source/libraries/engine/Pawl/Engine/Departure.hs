...
nonCardStackObjectsCease pid gs =
  let notACard oid = case Game.lookupObject oid gs of
        ...
        Just obj -> case Object.source obj of
          ...
          -- CR 108.2: represented by cards, so CR 800.4a's "objects on the
          ...
          -- copy of a spell is, so CR 800.4a reaches it too once CR 722.3c's or CR
          -- 707.12's permission has put it on the stack. Before that it sits in the
          -- zone it was made in, where the zone filter this feeds never looks.
...
--
-- CR 800.4h reaches this move through Replacement.chooserOf: the funnel's CR
