...
matchesTriggerGiven bindings board gs bearer you cond event = case cond of
  ...
  -- once per declared attacker; CR 508.4's creature put onto the battlefield
  ...
  -- putting the mark outside the permanent's copiable values.
  --
  ...
  TriggerCondition.SelfPutIntoGraveyardFromAnywhere -> case event of
    ...
    GameEvent.CardArrived zc ->
      ZoneChange.object zc == bearer
        && ZoneChange.to zc == Zone.Graveyard
  -- CR 702.55a's second sentence, which CR 608.2n dates: "when this spell is put
  -- into a graveyard during its resolution". The arm above's destination test
  -- plus the CAUSE, which is the whole of the difference -- a COUNTERED spell
  -- (CR 701.6a) and a FIZZLED one (CR 608.2b) reach the same graveyard from the
  -- same zone, and each extra fire would be an extra haunt in the controller's
  -- favour.
  --
  -- `from` is unread for the arm above's reason and for a second one: the cause
  -- already implies the stack, CR 608.2n's move having no other origin.
  --
  -- Matched on `object`, the arriving incarnation, for the arm above's reason --
  ...
  TriggerCondition.SelfPutIntoGraveyardDuringResolution -> case event of
    ...
    -- in which arrival they name. Pawl.MutateSpec's "from either arrangement"
    -- case proves it over both orders.
    -- Never CR 608.2n's move: rule 712.21's trailing arrivals are a melded
    -- PERMANENT's components leaving the battlefield, and a spell on the stack
    -- has none.
    GameEvent.CardArrived _ -> False
  ...
  -- more mana of the chosen color". The PermanentTappedForMana arm's posture --
  -- the adding player against `you` under the relation, the ability's source
  -- against the Filter, the added mana against the specification -- except
  -- that the player is the event's own recipient rather than a controller read
  -- off the board: CR 106.4 names whose pool the mana went to.
  TriggerCondition.AbilityAddsMana (AbilityAddsMana.MkAbilityAddsMana relation f specified) -> case event of
    GameEvent.ManaAdded added
  ...
  -- creature as it is tapped to pay a Vehicle's crew cost -- GameEvent.Crewed,
  -- written at the payment, and not the arm above's resolution. A bare
  -- membership test for that arm's reasons -- no view and no Filter.
  --
  ...
  --
  -- PlayerWinsCoinFlip's mirror, and the arm that makes CoinFlipped.won's Maybe
  -- observable: a winnerless flip answers False to both conditions, where a
  -- called-and-missed flip answers only this one.
  TriggerCondition.PlayerLosesCoinFlip relation -> case event of
    GameEvent.Moved {} -> False
    GameEvent.DamageDealt _ -> False
    GameEvent.StepBegan {} -> False
    GameEvent.SpellCast {} -> False
    ...
    GameEvent.ManaAdded _ -> False
    GameEvent.ManaAbilityResolved _ -> False
    -- A flip with NO outcome (CR 705.2's first sentence) is not a lost one
