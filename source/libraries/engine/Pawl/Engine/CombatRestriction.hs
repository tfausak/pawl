...
gathered gs =
  let -- Hoisted out of the walk as AttackRequirement.instances hoists them, and
      ...
      -- battlefield and none of an emblem (CR 114.5), a vanguard card (CR 313.2)
      -- and a conspiracy (CR 315.3) is one. The "unless" gate IS asked, because a
      -- gated restriction from this zone would otherwise be wired open -- nothing
      -- in the pool that functions here prints one.
      --
