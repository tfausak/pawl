            ...
            then reject
              ...
              if overCeiling || not (payableCost spending pid sid gs announcedAtX)
                then reject
                  ...
                  -- CR 601.2c, and the spell is on the stack for it: `sets` below
                  ...
                  -- CR 601.2b's X, handed to CR 601.2c as an ordinary binding: a
                  -- slot's own CR 202.3 computed bound may read it (Stir the
                  -- Grave's "mana value X or less"), and the value exists by now
                  -- because rule 601.2b announces the variable before this step.
                  -- Nothing else is seeded -- no target has been chosen yet, and
                  -- the modes are not a target slot's to read.
                  --
                  ...
                  -- the same set.
                  --
                  -- Measured on `bestowedGs`, the post-move board CR 601.2c is
                  -- judged against, as this binding was before it was moved below
                  -- the announcement.
