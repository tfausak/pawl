    ...
    if all (\c -> distinguishing c == distinguishing first) rest
      then pure (Just first)
        ...
        Just pid -> do
          ...
          answer <- Game.ask (Prompt.ChooseReplacement decider pid (fmap ReplacementCandidate.source candidates))
    ...
    (ReplacementEffect.EntryR _ rewrite, ProposedEvent.WouldEnter oid) -> case rewrite of
      EntryRewrite.AsCopy -> do
        ...
        case Projection.controllerOf oid gs of
          ...
          Just controller -> do
            ...
            answer <- Game.ask (Prompt.ChooseCopyTarget decider controller oid legal)
            ...
            picked <-
              if null rest
                ...
                -- nothing to ask, don't prompt.
                  ...
                  Just controller -> do
                    let decider = Decide.deciderFor controller gs
                    answer <- Game.ask (Prompt.ChooseEntryOption decider controller oid options)
      ...
      EntryRewrite.ChooseColor -> do
        ...
        picked <- case Projection.controllerOf oid gs of
          ...
          Just controller -> do
            let decider = Decide.deciderFor controller gs
            Game.ask (Prompt.ChooseColor decider controller oid)
        consume (ReplacementCandidate.identity candidate)
        State.modify' $ \g ->
      ...
      EntryRewrite.ChooseBasicLandType -> do
        ...
        picked <- case Projection.controllerOf oid gs of
          ...
          Just controller -> do
            let decider = Decide.deciderFor controller gs
            Game.ask (Prompt.ChooseBasicLandType decider controller oid)
        consume (ReplacementCandidate.identity candidate)
        State.modify' $ \g ->
          ...
          Just controller -> do
            ...
            opponent <- case opponents of
              ...
              first : second : rest -> do
                ...
                answer <- Game.ask (Prompt.ChooseOpponent (Decide.deciderFor controller gs) controller oid offered)
            ...
            let choosers = filter (\pid -> pid == controller || Just pid == opponent) (Game.apnapOrder gs)
                ask pid = Game.ask (Prompt.ChooseCardName (Decide.deciderFor pid gs) pid oid restriction)
...
askOne batch (pid, positions) = do
  ...
  case group of
    ...
    first : _ -> do
      let decider = Decide.deciderFor pid gs
      answer <- Game.ask (Prompt.OrderDamage decider pid group)
