...
-- leaves-the-battlefield abilities" -- and Pawl.Event's `leftBattlefield`
-- candidate source is its reader.
--
-- The two are the same value in the PROPOSED event Pawl.Replacement resolves
-- (Pawl.Type.ProposedEvent.WouldChangeZone): nothing has moved yet, so the
