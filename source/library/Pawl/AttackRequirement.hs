...
-- Pawl.BlockRequirement on the other side of the combat phase, and the third
-- module on the axis CR 613.11 reaches past the layer system: Pawl.PlayerEffect
-- answers rules questions about a PLAYER, Pawl.BlockRequirement the one CR 509.1c
-- asks about a pair of CREATURES, and this the one CR 508.1d asks about a single
-- one. None is a layer, and Pawl.Projection sees none of them -- CR 613.11 applies
-- all three "after all other continuous effects have been applied".
--
-- This module is the only reader of Pawl.Types.AttackRequirement. Pawl.Combat asks
-- for requirement INSTANCES -- bare creature ids -- and never learns which card
-- produced one, the same posture Pawl.Cast takes toward
-- Pawl.PlayerEffect.prohibitsCasting.
module Pawl.AttackRequirement where
...
-- Pawl.BlockRequirement.instances makes on identical pairs and costs no answer:
-- a declaration obeys both or neither.
--
-- `candidates` is CR 508.1a's chosen-from set (Pawl.Combat.legalAttackers), and
-- it carries the "if able" of "attacks each combat if able". Passed IN rather
-- than computed here, exactly as Pawl.BlockRequirement takes its `able`
-- predicate, so this module never learns the restrictions and Pawl.Combat stays
...
-- creature and already inside Pawl.Combat.canAttack, so the two collapse into
-- the one argument (#459).
instances :: [ObjectId] -> GameState -> Set ObjectId
instances candidates gs =
  let -- Hoisted out of the walk exactly as Pawl.BlockRequirement.instances hoists
      ...
      fromPermanent source = case Game.cardOf source gs of
        ...
        Just card -> case Card.attackRequirements card of
          ...
          [] -> []
          requirements ->
            -- The same TWO ability losses Pawl.BlockRequirement.instances asks
