...
- `test`:
  the test suite, including every engine spec

- `executable`:
  every entry point the package has, behind one dispatch on `argv`

Every module belongs to one of those. The `pawl`, `pawl-test-suite` and
...
above it in that table; adding an edge that inverts it means the module is in
the wrong place.
