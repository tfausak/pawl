...
Every library module lives under `source/library/` in the one `library` stanza
in `pawl.cabal`; only the executable, benchmark and test suite sit outside,
under `source/executable/`, `source/benchmark/` and `source/test-suite/`, each
a single `Main.hs`. There were once sublibraries, one per concern, and a
sublibrary could only depend on ones above it in the table below. They were
combined (#1880) because editing any module in one rebuilt every module of
every sublibrary downstream --- a type change rebuilt the whole codec and the
whole engine.

The layering they enforced survives as the module namespace, and is still how
you pick a name. Pick by what the module *is*, not by who calls it:
...
A module may only import ones above it in that table. Nothing enforces that
any more, so an import inverting it is a naming mistake the compiler will
accept --- watch for it in review rather than expecting a build error.
...
skipped, and deleting a module needs `cabal-gild pawl.cabal` run directly,
since `hooky fix` has skipped that case.

### Where the tests go

Every spec is an ordinary exposed module of the library, alongside the code it
covers; `source/test-suite/` holds only `Main.hs`, which aggregates them.
Shipping them this way is deliberate: `Pawl.Spec` is base-only, so they cost
essentially nothing and stay runnable by any consumer.

Outside the engine, the spec sits next to the module it covers
(`Pawl.Json.Value` -> `Pawl.Json.ValueSpec`) and exposes
...
In the engine, tests go in the subsystem spec directly under `Pawl.` that
near-mirrors the module under test (`Pawl.Engine.Foo` -> `Pawl.FooSpec` --- the
specs keep the shorter name, since the test suite covers more than the
...
comment listing the modules it covers. A new subsystem gets a new
`Pawl.<Area>Spec` wired into `Main.hs`'s `spec`, or into `testTree` if it needs
its own timeout.
