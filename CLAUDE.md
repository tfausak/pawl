...
Everything else goes in `source/library/` beneath `Pawl` — the engine, which
will become its own sublibrary once a second consumer exists. A sublibrary may
only depend on ones above it in that table; adding an edge that inverts it means
...
Two conventions, split by which half the module is in.

**Inside a sublibrary**, the spec sits next to the module it covers
...
**In the engine** (`source/library/`), tests go in the subsystem spec under
`source/test-suite/Pawl/` that near-mirrors the module under test (`Pawl.Foo` →
`Pawl.FooSpec`). Each exposes `tests :: TestTree` and heads with a comment
listing the modules it covers; `Main.hs` only aggregates them. A new subsystem
gets a new `Pawl.<Area>Spec` wired into `Main.hs`'s `testTree` and added to the
test-suite `other-modules` list. Shared fixtures live in `Pawl.Support`,
imported `qualified ... as S` — the one documented exception to
